...
const Filter = (props) => {
  ...
  const uniqueLocationOfOrigin = [...new Set(props.rawItems.map(item => item.origin_colony))];
  const uniqueMAS = [...new Set(props.rawItems.map(item => item.mas))];
  const uniqueCreator = [...new Set(props.rawItems.map(item => item.creator))];
  const uniqueProv = [...new Set(props.rawItems.map(item => item.prov))];
  ...
  // Location of Origin
  const [locOfOrigin, setLocOfOrigin] = React.useState([]);
  const handleLocOfOriginChange = (event) => {
    const {
      target: { value },
    } = event;
    setLocOfOrigin(
